...
import { AddRoute, GetAssignedCustomers } from "../../redux/reducers/AccounsLogin/VehicleDetails";
...
const AddAssignedDriver = () => {
    ...
    // to filter the the customer 
    const assignedCustomerIds = useMemo(() => {
        let ids = [];
        GetAssignRouteData?.forEach(route => {
            route.customers?.forEach(cust => {
    ...
    const customerDropdownData = useMemo(() => {
        return CustomersDataGetCall
            ?.filter(item => !assignedCustomerIds.includes(item.id)) // ❌ remove already assigned
            ?.map(item => ({
                label: item.ownerName,
                value: item.id,
            })) || [];
    }, [CustomersDataGetCall, assignedCustomerIds]);
    ...
    const handleSubmit = async () => {
        ...
        console.log("Payload Data  that dispatcing  UI Screen Add Assigned Route ----------------------->", payload);

        try {
            const response = await dispatch(AddRoute(payload)).unwrap();  //fulfilled → returns actual payload  //rejected → throws error to catch block
...
export default AddAssignedDriver