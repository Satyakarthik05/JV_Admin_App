...
import { StyleSheet, Text, View, TouchableOpacity, TextInput, StatusBar, Alert, ActivityIndicator, ScrollView, KeyboardAvoidingView, Platform, TouchableWithoutFeedback, Keyboard } from "react-native";
import { SafeAreaView, useSafeAreaInsets } from "react-native-safe-area-context";
import Feather from 'react-native-vector-icons/Feather';
import { colors, fonts } from "../../config/theme";
import { useDispatch, useSelector } from "react-redux";
import { AddVehicleDetailsPostCall, GetVehiclesData } from "../../redux/reducers/AccounsLogin/VehicleDetails";
...
const AddVehicleDetails = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await dispatch(AddVehicleDetailsPostCall(payload)).unwrap();  //fulfilled → returns actual payload  //rejected → throws error to catch block
                        ...
                        {/* <View style={[styles.bottomContainer, { paddingBottom: insets.bottom + 10 }]}>
                            <TouchableOpacity onPress={handleSubmit} disabled={isSubmitting} style={[styles.button, isSubmitting && { opacity: 0.6 }]}>
                                {isSubmitting ? (
                                    ...
                                    <Text style={styles.btnText}>Add Vehicle Details</Text>
                                )}
                            </TouchableOpacity>
                        </View> */}
