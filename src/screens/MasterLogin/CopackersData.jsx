...
import { DeleteCopacker, GetCopackers } from "../../redux/reducers/MasterLogin/AddProduct";
...
const CopakersData = () => {
    ...
    const onRefresh = async () => {
        setRefreshing(true);
        try {
            await dispatch(GetCopackers()).unwrap();
        } catch (error) {
    ...
    const handleDelete = (id) => {
        Alert.alert(
            "Delete the copacker", "Are You Sure To Delete  copacker",
            [
                {
                    text: "No",
                ...
                },
                {
                    text: "Yes",
                    onPress: async () => {
                        await dispatch(DeleteCopacker(id)).unwrap();
                        dispatch(GetCopackers());
                    }
                }
            ],
    ...
    console.log("Get copackersdata  ==============>", copackersData);
    ...
    console.log("Logined User Data async storege in Co packers Screen --------------------->", userData);
        ...
        <View style={styles.container}>
            <SafeAreaView style={styles.top}>
                <TouchableOpacity style={commonstyles.header} onPress={() => navigation.goBack()}>
                    ...
                    <Text style={commonstyles.title}>Co Packers Data </Text>
                </TouchableOpacity>
                {
                    !isAdmin && (
                        <TouchableOpacity style={styles.button} onPress={() => navigation.navigate("AddCopackers")}>
                            <Text style={styles.btn_text}>Add Co Packer</Text>
                        </TouchableOpacity>
                    )
                }
                        ...
                        return (
                            <TouchableOpacity style={styles.card} >
                                <View style={styles.top}>
                                    <Text style={styles.text}><Text style={styles.in}>Name:</Text>{item.copackerName}</Text>
...

                                {
                                    !isAdmin && (
                                        <View style={{ flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-end', gap: responsiveHeight(2) }}>
                                            <TouchableOpacity onPress={() => navigation.navigate("editCopackers", { editdata: item })}>
