...
import { AddSupplierInfo } from "../../redux/reducers/MasterLogin/AddProduct";
...
const AddSupplier = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            console.log("Supplier Payload  Dispatch in UI --->", payload);

            //  API call here
            const response = await dispatch(AddSupplierInfo(payload)).unwrap();

            Alert.alert(
                "Success", response?.data?.message || response?.message || "Added  Supplier Information Successfully",
                [
                    {
                        text: "OK",
        ...
        <View style={styles.container}>
            ...
            <SafeAreaView>
                <TouchableOpacity style={commonstyles.header} onPress={() => navigation.goBack()}>
                    ...
                    <Text style={commonstyles.title}>Add Supplier Information</Text>
                </TouchableOpacity>
            </SafeAreaView>
            ...
            >
                <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    <View style={[styles.inputBox, errors.supplierName && styles.errorBorder]}>
                        <TextInput placeholder="Enter supplier name" value={supplierName} placeholderTextColor={colors.inputfieldcolor}
                            onChangeText={(text) => {
                    ...
                    <View style={[styles.inputBox, errors.altmobile && styles.errorBorder]}>
                        <TextInput placeholder="Enter alternative mobile number" keyboardType="number-pad" maxLength={10} value={altmobile}  placeholderTextColor={colors.inputfieldcolor}
                            onChangeText={(text) => {
                                setaltMobile(text);
                                if (errors.altmobile) {
                    ...
                    <View style={[styles.inputBox, errors.email && styles.errorBorder]}>
                        <TextInput placeholder="Enter email" value={email} placeholderTextColor={colors.inputfieldcolor}
                            onChangeText={(text) => {
                                setEmail(text);
                ...
                <View style={[styles.bottom, { paddingBottom: insets.bottom + 10 }]}>
                    <TouchableOpacity onPress={handleSubmit} disabled={isSubmitting} style={[styles.button, isSubmitting && { opacity: 0.6 }]}  >
                        {isSubmitting ? (
                            ...
                            <Text style={styles.btnText}>Add Supplier Information</Text>
                        )}
                    </TouchableOpacity>
                </View>
            </KeyboardAvoidingView>
