...
import { EditAccessoryInventoryData, GetAccessoriesInAcc } from "../../redux/reducers/MasterLogin/AddProduct";
import DateTimePicker from '@react-native-community/datetimepicker';
import commonstyles from "../../commonstyles/commonstyles";



const EditInventoryControl = () => {
    ...
    //const [date, setDate] = useState(formatDate(editData?.purchaseDate));
    const [date, setDate] = useState(convertToLocalDate(editData?.purchaseDate));
    ...
    const handleDateChange = (event, selectedDate) => {
        ...
        if (selectedDate) {
            ...
            setDate(`${year}-${month}-${day}`); // API format
        }
    };
    ...
    const [invoice, setInvoice] = useState(editData?.invoiceNumber);
    ...
    const [accessories, setAccessories] = useState(
        editData?.items?.map((item, index) => ({
            id: item.itemId,
            category: item.accessoryTypeId,//Accessory typevalue
            materialno: index + 1,
            name: item.accessoryId, //Accessory name
            // unit: '',
    ...
    const AccessoryName =
        GetAccessoryData?.map(item => ({
            ...
            value: item.id,   //  use id
            // value: item?.accessoryName,//uses name
            // id: item.id,
        })) || [];
    ...
    const handleValidation = () => {
        ...
        // if (!vendorname.trim()) newerror.vendorname = "Please Enter Vendor Name";
    ...
    const handleUpdate = async () => {
        ...
        const payload = {
            //purchaseDate: editData?.purchaseDate?.split("T")[0], // or use current date
            purchaseDate: date,
        ...
        console.log("Final Payload -------------------->", payload, id);
        try {
            const response = await dispatch(EditAccessoryInventoryData({ id, payload })).unwrap();
            Alert.alert(
                "Success", response?.data?.message || response?.message || "Updated  Accessory Inventory Successfully",
                [
                    {
                        text: "OK",
        ...
        <View style={styles.container}>
            ...
            <SafeAreaView >
                <TouchableOpacity style={commonstyles.header} onPress={() => navigation.goBack()}>
                    ...
                    <Text style={commonstyles.title}>Edit Inventory Control</Text>
                </TouchableOpacity>
            </SafeAreaView>
            ...
            <ScrollView style={styles.main} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.head}>
                    ...
                    <View style={[styles.for_border, validerror?.invoice ? styles.errorBorder : null]}>
                        <TextInput placeholder="In-voice Number " style={styles.inputfield} placeholderTextColor="#888" value={invoice} onChangeText={setInvoice} />
                    </View>
                    {
                        validerror.invoice ? (
                ...
                <View style={styles.head}>
                    ...
                    <View style={[styles.for_border, validerror?.vendorname ? styles.errorBorder : null]}>
                        <TextInput placeholder="Vendor Name" style={styles.inputfield} placeholderTextColor="#888" value={vendorname} onChangeText={setVendorName} />
                    </View>
                    {
...
export default EditInventoryControl
