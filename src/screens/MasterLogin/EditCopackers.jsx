...
import { EditCoPackerData } from "../../redux/reducers/MasterLogin/AddProduct";
...
const EditCopackers = () => {
    ...
    console.log("Edit data in params----------------->", editdata);


    const [isSubmitting, setIsSubmitting] = useState(false);

    const [supplierName, setSupplierName] = useState(editdata?.copackerName|| '');
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            console.log("edit copackers Payload data--------------------->", payload,id);

            const response = await dispatch(EditCoPackerData({id,payload})).unwrap();
            Alert.alert(
                "Success", response?.data?.message || response?.message || "Updated Successfully",
                [
                    {
                        text: "OK",
        ...
        <View style={styles.container}>
            ...
            <SafeAreaView>
                <TouchableOpacity style={commonstyles.header} onPress={() => navigation.goBack()}>
                    ...
                    <Text style={commonstyles.title}>Edit Co Packer</Text>
                </TouchableOpacity>
            </SafeAreaView>
            ...
            >
                <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    <View style={[styles.inputBox, errors.supplierName && styles.errorBorder]}>
                        <TextInput placeholder="Enter name" value={supplierName}
                            onChangeText={(text) => {
                ...
                <View style={[styles.bottom, { paddingBottom: insets.bottom + 10 }]}>
                    <TouchableOpacity onPress={handleSubmit} disabled={isSubmitting} style={[styles.button, isSubmitting && { opacity: 0.6 }]}  >
                        {isSubmitting ? (
                            ...
                            <Text style={styles.btnText}>Update Co Packer</Text>
                        )}
                    </TouchableOpacity>
                </View>
            </KeyboardAvoidingView>
