...
import { EditPurchase, GetAllRawMaterialData, GetSupplierInformationData } from "../../redux/reducers/MasterLogin/AddProduct";
...
const Editpurchase = () => {
    ...
    console.log("Purchase Data coming fromgey call to edit call --------------->", purchaseData);
    ...
    console.log("Get Supplier Information Data In Edit  Purchase Screen  ===============>", SupplierInfoData);

    const { UnitsData } = useSelector((state) => state.GetUnitsRawMaster);
    console.log("Get Units  data in  Edit Purchase Data Screen ===============>", UnitsData);
    ...
    const [date, setDate] = useState(null);// stored seleced data
    ...
    const removeproduct = (id) => {
        if (product.length === 1) {
            Alert.alert("At least one product is required");
            return;
        }
    ...
    const validationError = () => {
        ...
        product.forEach((item, index) => {
            // if (!item.name) newerror[`material_${index}`] = "Please select Product Name ";
            if (!item.rawMaterialId) newerror[`material_${index}`] = "Please select Product Name ";
            // if (!item.unit?.trim()) newerror[`unit_${index}`] = "Please Enter Unit Price";
            // if (!item.quantity || item.quantity?.trim() === " ") newerror[`quantity_${index}`] = "Please Enter Quantity";
            // if (!item.cost || item.cost?.trim() === " ") newerror[`cost_${index}`] = "Please Enter Purchase Cost";
    ...
    useEffect(() => {
        if (purchaseData) {
            ...
            //  FIX DATE HERE
            const localDate = new Date(purchaseData.purchaseDate);
            ...
            setTotalAmount(String(purchaseData.totalAmount));
            setRemarks(purchaseData.remarks);

            if (purchaseData.items) {

                const formattedProducts = purchaseData.items.map((item, index) => ({
                    id: Date.now() + index,
                    //category: item.category || null,
                    categoryId: item.categoryId
                        ?? GetRawCategory?.find(cat => cat.categoryName === item.category)?.id
                        ?? null,
                    ...
                    rawMaterialId: item.rawMaterialId,          // product name 
                    rawMaterialCode: item.rawMaterialName,      // its id 
                    //unit: String(item.unitPrice),
                    quantity: String(item.quantity),
                    cost: String(item.totalPrice)
                }));

                setProduct(formattedProducts);
                setMaterialCount(formattedProducts.length);
            }
        }
    }, [purchaseData]);
    ...
    const handleSubmit = async () => {
        ...
        const payload = {
            // for supplier name send to palyod 
            ...
            items: product
                ...
                .filter(item => item.rawMaterialId && item.quantity && item.cost)
                .map((item, index) => ({
                    //rawMaterialId: index + 1,
                    rawMaterialId: item.rawMaterialId,
                    //rawMaterialCode: item.name,
                    rawMaterialCode: item.rawMaterialCode.trim(),
        ...
        console.log("Payload data that dispatching  to redux code Edit Purchase  code to redux code -------------------->", payload, id);
        try {

            const response = await dispatch(EditPurchase({ id, payload })).unwrap();
            Alert.alert(
        ...
        <View style={styles.container}>
            ...
            <SafeAreaView >
                <TouchableOpacity style={commonstyles.header} onPress={() => navigation.goBack()}>
                    ...
                    <Text style={commonstyles.title}>Edit Purchase</Text>
                </TouchableOpacity>
            </SafeAreaView>
            ...
            <ScrollView style={styles.main} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.head}>
                        ...
                        <View style={[styles.for_border, validerror?.date ? styles.errorBorder : null]}>
                            <TextInput style={styles.inputfield} editable={false} value={date ? formatDate(date) : ""} placeholder="Date" placeholderTextColor="#888" />
                    ...
                    <View style={styles.head}>
                        ...
                        <View style={[styles.for_border, validerror?.totalAmount ? styles.errorBorder : null]}>
                            <TextInput placeholder="Total Amount" style={styles.inputfield} placeholderTextColor="#888" value={totalAmount} editable={false} keyboardType="numeric" />
                        </View>
                        {
                    ...
                    <View style={styles.head}>
                        ...
                        <View style={[styles.for_border, validerror?.remarks ? styles.errorBorder : null]}>
                            ...
                            <View style={styles.card} key={index}>
                                                ...
                                                setProduct(prev =>
                                                    prev.map(prod =>
                                                        prod.id === item.id
                                                            ? {
                                                                ...prod,
                                                                rawMaterialId: val.value,
                                                                rawMaterialCode: val.label.trim()
                                                            }
                                                            : prod
                                                    )
                                                )
                                ...
                                {/* <View style={styles.head}>
                                    ...
                                    <View style={[styles.for_newborder, validerror[`quantity_${index}`] ? styles.errorBorder : null]}>
                                        <TextInput placeholder="Product Quantity" style={styles.inputfield} value={item.quantity} keyboardType="numeric"
                                            onChangeText={text => {
                                ...
                                <View style={styles.head}>
                                    ...
                                    <View style={[styles.for_newborder, validerror[`cost_${index}`] ? styles.errorBorder : null]}>
                                        <TextInput placeholder="Purchase Cost" style={styles.inputfield} value={item.cost} keyboardType="numeric"
                                            onChangeText={text => {
                ...
                {/* <TouchableOpacity style={styles.button} onPress={handleSubmit}>
                    <Text style={styles.btn_text}>Update Purchase</Text>
                </TouchableOpacity> */}
            ...
            </ScrollView>
            {
                showDate &&
                <DateTimePicker
                    value={new Date()}// new Date(date) we need ti change i'm thinking
                    mode="date"
                    display="default"
